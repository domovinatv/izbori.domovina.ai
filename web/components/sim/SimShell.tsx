...
export function SimShell({ cycles }: { cycles: RawCycle[] }) {
  ...
  return (
    <div className="flex h-full w-full flex-col text-ink">
      {/* ── Header ─────────────────────────────────────────────────────── */}
      <header className="flex h-[46px] shrink-0 items-center gap-3 border-b border-line bg-navy-deep px-3 text-hero-ink">
        <Link
          href="/"
          className="flex items-center gap-2 transition-opacity hover:opacity-80"
          title="Natrag na izbori.domovina.ai"
        >
          <span className="tricolor-rule tricolor-rule--dark !w-8" aria-hidden="true" />
        ...
        {preset ? (
          <span className="flex min-w-0 items-center gap-1.5">
            ...
            <span className="truncate text-[10px] text-hero-ink/75" title={preset.opis}>
              {preset.opis}
            </span>
          </span>
        ) : null}
      ...
      {/* ── Body ───────────────────────────────────────────────────────── */}
      <div className="grid min-h-0 flex-1 grid-cols-[286px_minmax(0,1fr)_330px] gap-2 p-2">
        {/* Left rail */}
        <div className="grid min-h-0 grid-rows-[minmax(0,1.05fr)_minmax(0,1fr)] gap-2">
          ...
          <Panel
            ...
            badge={<SourceBadge source="KONSTRUKCIJA" title="Pretpostavka koju zadaješ ti — nije prognoza" />}
            bodyClassName="overflow-y-auto"
          >
            <SwingPanel
              cycle={cycle}
            ...
            {tab === "mandati" ? (
              <div className="grid h-full grid-rows-[minmax(0,auto)_minmax(0,1fr)] gap-2">
                <Panel bodyClassName="flex items-start justify-around gap-4 px-3 py-2">
                        ...
                        {promjene.map((p) => (
                          ...
                          >
                            <span
                              ...
                              style={{ color: p.delta > 0 ? "#1F6B33" : "#B42318" }}
                            >
                              {p.delta > 0 ? `+${p.delta}` : p.delta}
                            </span>
                            <span className="truncate text-muted" title={p.obitelj}>
                  ...
                  <Hemicycle
                    res={res}
                    naslov={izmijenjeno ? "Simulacija" : "Simulacija (bez izmjena)"}
                    hover={hover}
                    onHover={setHover}
                    height={172}
                  />
                ...
                >
                  <ListDeltaTable res={res} hover={hover} onHover={setHover} />
                </Panel>
              </div>
            ) : null}
          ...
          <Panel title="Izvori i ograničenja" bodyClassName="overflow-y-auto">
            <div className="flex flex-col gap-1.5 px-3 py-2 text-[9px] leading-relaxed text-muted">
              <p>
                ...
                Ustav čl. 45 (dijaspora 3), čl. 41 (prag 5 %), čl. 40 (D&apos;Hondt,
                preferencijali 10 %), čl. 17 i 46 (8 manjinskih, većinski).
              </p>
              <p>
                <SourceBadge source="PODACI" /> DIP arhiva, mirrorana lokalno.
                Odaziv i birači obuhvaćaju IJ I–XI; javno citirani odaziv uključuje
                i XII. pa je nešto viši.
              </p>
              <p>
                <SourceBadge source="PRETPOSTAVKA" /> {TIE_BREAK_ASSUMPTION}
              </p>
              <p className="border-t border-line/70 pt-1.5">
                <strong className="text-ink">Obitelj</strong> je vodeća stranka
                koalicijske liste, ne stranačka pripadnost osobe. Ista koalicija
                nastupala je pod različitim nazivima u različitim jedinicama, pa
                se ovdje spaja po vodećoj stranci.
              </p>
              <p>
                <strong className="text-ink">Što ovo ne radi:</strong> ne
                predviđa glasove, ne modelira kampanju i ne zna koje su koalicije
                politički moguće. Pomak glasova je pretpostavka koju zadaješ ti.
              </p>
              <p>
                Puna činjenična osnova s citatima:{" "}
                <code className="text-ink">docs/izborni_sustav_cinjenice.md</code>
              </p>
            </div>
          </Panel>
        </div>
